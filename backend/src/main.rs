...
async fn invoke(Path(cmd): Path<String>, Json(args): Json<Value>) -> impl IntoResponse {
    ...
    let result: Result<Value, String> = match cmd.as_str() {
        ...
        "lan_local_info" => modules::lan_map::lan_local_info().await,
        "repeater_send" => call!(modules::repeater::repeater_send, app, args, "req"),
        "repeater_to_curl" => {
        ...
        "dns_query" => call!(modules::dns_tools::dns_query, app, args, "req"),
        "ssl_scan" => call!(modules::ssl_scan::ssl_scan, app, args, "req"),
        "banner_grab" => call!(modules::banner::banner_grab, app, args, "req"),
        "iana_tld_list" => modules::domain_grabber::iana_tld_list().await,
        "domain_grab" => call!(modules::domain_grabber::domain_grab, app, args, "req"),
        "wordlist_info" => Ok(modules::domain_grabber::wordlist_info()),
        "xploit_store_init" => call!(modules::xploiter_store::xploit_store_init, app, args, "none"),
        "xploit_store_list" => {
